...
character - one of nine archetypes, a head out of that archetype's own
handful, a palette, a voice and a name - react to what happens in
...
`render/mixer.ts` touches the DOM, so all of it is unit-tested - the sprite
...
- There are no image files to publish. The sprite art is code
  (`render/mixer-art.ts`, baked by `yarn mixer`), which the registry compiles
...
yarn mixer heads out.png                     all 333 heads, labelled with their index
...
| `combatState` - in a fight, and out of it | `guard` / `guardStaff` / `guardBow`, held for the fight | *(silent)* | 0 |
...
`guardStaff` for the two robed archetypes, `guardBow` for the archer, and
`guard` - the same sword the `lunge` swings - for everybody else. So the
weapon comes out when the fight starts and goes away when it ends.
